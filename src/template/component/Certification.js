...
const Certification = ({certi_data}) => {
  return (
    ...
    fontWeight: '500'}}>Certification</Typography></div>
    {certi_data.map((data, i) => {
      ...
      <Grid item sm={4}><Typography sx={{fontSize:'11px', textAlign:'right'}}>{data.course_start} - {data.course_end}</Typography></Grid>
      <Grid item sm={12}><Typography sx={{fontSize:'12px', fontWeight:'500', color:'#93899d'}}>{data.org_name}</Typography></Grid>
          {course_summery.blocks.map((role_data, j) => (
              <Typography key={j}>{role_data.text}</Typography>
          ))}
  </Grid>)
    })}
    </>
