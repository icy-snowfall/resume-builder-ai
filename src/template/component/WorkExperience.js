...
const WorkExperience = ({exp_data}) => {
  return (
    ...
    <Typography className='tlt_text'>WorkExperience</Typography></div>
    {exp_data.map((data, i) => {
      ...
      return(
        <Grid container key={i} sx={{p:'10px'}}>
            ...
            <Grid item sm={4}><Typography className='para_text' sx={{textAlign:'right'}}>{data.work_start} - {data.work_leave}</Typography></Grid>
